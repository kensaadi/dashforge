...
describe('OTPField precedence chain — Option C (Track A)', () => {
  it('level 1 — TV defaults renders', () => {
    const { container } = render(<OTPField name="otp" label="Code" length={4} />);
  ...
  it('level 2 — theme override wins', () => {
    act(() => {
      ...
      (patchTheme as any)({
        components: { OTPField: { defaults: { size: 'lg' } } },
      });
    });
    const { container } = render(<OTPField name="otp" label="Code" length={4} />);
    expect(container.querySelector('input')).not.toBeNull();
  ...
  it('level 3 — instance prop wins', () => {
    ...
    const { container } = render(<OTPField name="otp" label="Code" length={4} size="sm" />);
    expect(container.querySelector('input')).not.toBeNull();
  });

  it('level 4 — sx wins via tailwind-merge (root)', () => {
    const { container } = render(<OTPField name="otp" label="Code" length={4} sx="bg-yellow-100" />);
