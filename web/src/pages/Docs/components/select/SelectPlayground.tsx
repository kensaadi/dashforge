...
import {
  ...
  type FieldLayout,
  type SelectVariant,
} from './selectPlayground.helpers';

/**
 * SelectPlayground provides an interactive playground for Select component
...
export function SelectPlayground() {
  ...
  const [state, setState] =
    useState<SelectPlaygroundState>(DEFAULT_SELECT_STATE);
  ...
  const handleChange = (field: keyof SelectPlaygroundState) => {
    return (
  ...
  const handleVariantChange = (newVariant: SelectVariant) => {
    setState((prev) => ({
      ...prev,
      variant: newVariant,
    }));
  ...
  const handleReset = () => {
    setState(DEFAULT_SELECT_STATE);
    setActivePresetId('basic');
  };

  const handlePresetClick = (presetId: string) => {
    const preset = SELECT_PRESETS.find((p) => p.id === presetId);
    if (preset) {
        ...
        >
          {(['outlined', 'filled', 'standard'] as const).map((variant) => (
            <Button
              ...
              variant={state.variant === variant ? 'contained' : 'outlined'}
              sx={{
                bgcolor:
