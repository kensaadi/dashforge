...
import {
  generateNumberFieldCode,
  DEFAULT_NUMBERFIELD_STATE,
  NUMBERFIELD_PRESETS,
  type NumberFieldPlaygroundState,
  type NumberFieldVariant,
} from './numberFieldPlayground.helpers';

/**
 * NumberFieldPlayground provides an interactive playground for NumberField component
 * Users can modify props through controls and see live updates
 */
export function NumberFieldPlayground() {
  const dashTheme = useDashTheme();
  const isDark = dashTheme.meta.mode === 'dark';
  const [state, setState] = useState<NumberFieldPlaygroundState>(
    DEFAULT_NUMBERFIELD_STATE
  );
  const [activePresetId, setActivePresetId] = useState<string>('basic');

  const handleChange = (field: keyof NumberFieldPlaygroundState) => {
    return (
  ...
  const handleVariantChange = (newVariant: NumberFieldVariant) => {
    setState((prev) => ({
      ...prev,
      variant: newVariant,
    }));
  ...
  const handleReset = () => {
    setState(DEFAULT_NUMBERFIELD_STATE);
    setActivePresetId('basic');
  };

  const handlePresetClick = (presetId: string) => {
    const preset = NUMBERFIELD_PRESETS.find((p) => p.id === presetId);
    if (preset) {
  ...
  const controls = (
    <Stack spacing={2.5}>
      ...
      <Box>
        ...
        >
          {NUMBERFIELD_PRESETS.map((preset) => {
  ...
  const code = generateNumberFieldCode(state);
